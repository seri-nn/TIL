...
#### 8진수와 16진수로 데이터 표현
C언어는 기본적으로 10진수 이외에 8진수와 16진수 표현도 허허용함 
...
![image](https://github.com/seri-nn/TIL/assets/129299033/71f11299-a2b2-4627-9a45-0bbc6e557968)
...
아래 수식을 이용해 적은 비트 수를 가지고 보다 넓은 범위의 실수를 표현할 수 있음 
...
- '^' : 비트 단위로 xor 연산 (두 수가 같다면 0, 두 수가 다르면 1 반환)
  ...
  - 1 ^ 0 -> 1
  - 1 ^ 1 -> 0
- '~' : 단항 연산자(한 개의 행만 필요)로서 피연산자의 모든 비트를 반전 (반전 결과만 반환) **1의 보수를 취하는 효과**
  - ~ 0 : 1
  - ~ 1 : 0
...
{
    int n1 = 30; 
    int n2 = ~n1;  // 1의 보수를 취하는 효과이고 1의 보수 + 1 = 2의 보수(음의 정수)인데 아직 +1을 하기 전이니까 -31 (원래 not이면 -30이 나와야함 근데 +1은 아직 안했으니까 -31)
